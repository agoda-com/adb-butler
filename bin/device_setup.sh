...
function setup_emulator {
  ...
  if [ -z "${MARATHON_SERIAL}" ]; then
    ...
    if [ -n "${EMULATOR_TIMEZONE}" ]; then
      timeout -t 30 adb -s "${DEVICE}" shell su root setprop persist.sys.timezone "${EMULATOR_TIMEZONE}"
    fi
    if [ -n "${HTTP_PROXY}" ]; then
      timeout -t 30 adb -s "${DEVICE}" shell settings put global http_proxy "${HTTP_PROXY}"
    fi
...
while sleep 1; do
  echo -n | adb devices | grep -E 'device$' | awk '{ print $1 }' | sort >$DL.new
  diff -u $DL $DL.new | grep '^[+][^+]' | sed -E 's/^\+//' | while read -r d; do
  ...
  if [ -n "${STF_PROVIDER_PUBLIC_IP}" ]; then
